...
class Sprite:
    ...
    def collide(self, other_object):
        #if math.sqrt(other_object.get_position()[1] - self.get_position()[1])**2 + (other_object.get_position()[0] - self.get_position()[0])**2 <= other_object.get_radius()+self.get_radius():
            #print True
...
def rock_spawner():
    #global a_rock
    global rock_group, canvas, started, my_ship
    #i = 0
    ...
    if started == True:
        if len(rock_group) <12:
            ...
            rock_avel = random.random() * .2 - .1
            a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
            if dist(a_rock.get_position(), my_ship.get_position()) > a_rock.get_radius() + my_ship.get_radius() + 30:
                rock_group.add(a_rock)
            soundtrack.play()
            #i += 1
        #process_sprite_group(rock_group, canvas)
